...
unsigned int* get_v_ms2pip(int peplen, unsigned short* peptide, unsigned short* modpeptide, int charge, unsigned short* mod_features)
	{
	int i,j,k;
	int num_mod_features = 12;  // Be sure to change if more mod_features are added!
	float mz;
	...
	for (i=0; i < peplen-1; i++) {
		...
		for (k=0; k < num_mod_features; k++) {
			v[fnum++] = mod_features[k * (peplen - 1)  + i];
		}
...
float* get_p_ms2pip(int peplen, unsigned short* peptide, unsigned short* modpeptide, int charge, unsigned short* mod_features)
	...
	unsigned int* v = get_v_ms2pip(peplen, peptide, modpeptide, charge, mod_features);

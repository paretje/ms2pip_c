# For passing of two dimensional array, see example:
...
cdef extern from "ms2pipfeatures_c_HCDphospho.c":
	...
	unsigned int* get_v_ms2pip(int peplen, unsigned short* peptide, unsigned short* modpeptide, int charge, unsigned short* mod_features)
	...
	float* get_p_ms2pip(int peplen, unsigned short* peptide, unsigned short* modpeptide, int charge, unsigned short* mod_features)
...
def get_vector(
	np.ndarray[unsigned short, ndim=1, mode="c"] peptide,
	np.ndarray[unsigned short, ndim=1, mode="c"] modpeptide,
	charge,
	np.ndarray[unsigned short, ndim=2, mode="c"] mod_features):

	cdef unsigned int* results = get_v_ms2pip(len(peptide)-2,&peptide[0],&modpeptide[0],charge,&mod_features[0,0])
...
def get_predictions(
	...
	cdef float* results = get_p_ms2pip(len(peptide)-2,&peptide[0],&modpeptide[0],charge,&mod_features[0,0])
